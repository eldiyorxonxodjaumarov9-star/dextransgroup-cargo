...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Acknowledge malformed payloads without revealing details
  ...
  try {
    ...
    await handleTelegramStartUpdate({
      update,
      appUrl,
      botToken,
      adminIds,
  ...
  // Always 200 for accepted webhook deliveries (idempotent /start safe)
  return NextResponse.json({ ok: true });
}
