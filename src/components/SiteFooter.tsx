...
export function SiteFooter() {
  ...
  return (
    <footer className="mt-8 w-full min-w-0 max-w-full border-t border-[var(--border)] bg-[var(--background)] text-[var(--text)] safe-bottom">
      <div className="canvas-pad py-12 sm:py-20 lg:py-24">
        <div className="grid w-full min-w-0 grid-cols-1 gap-10 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
          <div className="min-w-0 space-y-6 sm:space-y-8">
            ...
            <p className="max-w-full break-words font-display text-[clamp(1.7rem,7vw,4rem)] font-semibold leading-[1.08] tracking-[-0.035em] sm:max-w-[16ch]">
          ...
          <div className="hidden gap-10 sm:grid sm:grid-cols-2">
            <div>
              <p className="mb-5 tracking-id text-[10px] text-[var(--text-muted)]">
                {t.footer.menu}
              </p>
              <ul className="space-y-1">
                {footerLinks.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="inline-flex min-h-11 items-center text-[var(--text-secondary)] transition hover:text-[var(--accent)]"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="space-y-2 sm:hidden">
            {(
              ...
              >
                ...
                {open === block.key && (
                  <ul className="space-y-1 border-t border-[var(--border)] px-4 py-3 text-sm text-[var(--text-secondary)]">
                      ...
                      <li key={item.href}>
                        {"external" in item && item.external ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex min-h-11 items-center break-all text-[var(--text-secondary)] transition hover:text-[var(--accent)]"
                          >
                            {item.label}
                          </a>
                        ) : (
      ...
      <div className="border-t border-[var(--border)] px-5 py-5 text-center text-xs leading-relaxed text-[var(--text-muted)] sm:px-10">
