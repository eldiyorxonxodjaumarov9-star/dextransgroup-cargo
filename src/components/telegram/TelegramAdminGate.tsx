...
export function TelegramAdminGate() {
  ...
  return (
    <div className="mx-auto flex min-h-[70dvh] w-full min-w-0 max-w-md flex-col items-center justify-center gap-4 px-4 py-16 text-center">
      <div className="w-full rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface)] p-6">
        <p className="section-kicker">Telegram Admin</p>
        <h1 className="mt-3 font-display text-2xl font-semibold tracking-tight text-[var(--text)]">
          Admin panel
        </h1>
        <p
        ...
        {(status === "booting" ||
          ...
          status === "redirecting") && (
          <div
            className="mx-auto mt-6 h-10 w-10 animate-pulse rounded-full bg-[color-mix(in_srgb,var(--accent)_40%,transparent)]"
            aria-hidden
          />
        )}
      </div>
