...
export function ItemsManager({
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="card grid gap-4 rounded-[var(--radius-xl)] p-4 sm:p-5 md:grid-cols-2">
        <div className="md:col-span-2 space-y-3">
          <h2 className="font-display text-xl font-semibold tracking-tight">
            {editingId ? "Tovarni tahrirlash" : "Yangi tovar qo‘shish"}
          </h2>

          <fieldset className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-soft)] p-4">
            <legend className="px-1 text-sm font-semibold text-[var(--text-secondary)]">
              Ma’lumot kiritish turi
            </legend>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border bg-card px-3 py-3 has-[:checked]:border-[var(--accent)] has-[:checked]:bg-[color-mix(in_srgb,var(--accent)_18%,transparent)]">
                <input
                  type="radio"
                  name="itemEntryType"
        ...
        {entryType === "PDF" ? (
          <div className="md:col-span-2 space-y-3">
            <label
              className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-[var(--accent)]/50 bg-[color-mix(in_srgb,var(--accent)_18%,transparent)] px-4 py-10 text-center transition hover:border-[var(--accent)]"
      ...
      <div className="card overflow-hidden rounded-[var(--radius-xl)]">
        <div className="flex flex-col gap-3 border-b border-[var(--border)] p-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          <h2 className="font-display text-xl font-semibold tracking-tight">Tovarlar ro‘yxati</h2>
          <select
            value={filter}
            onChange={(e) => setFilter(e.target.value)}
            className="min-h-11 w-full rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface-soft)] px-3 py-2 text-base sm:w-auto sm:text-sm"
          >
        ...
        <div className="space-y-3 p-3 md:hidden">
          {filtered.map((item) => (
            <div key={item.id} className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-elevated)] p-3">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${
                        item.entryType === "PDF"
                          ? "bg-[var(--surface-soft)] text-[var(--accent-secondary)]"
